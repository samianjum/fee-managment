...
def add_student(request, schema_name):
    ...
    with schema_context(schema_name):
        ...
        context = {'tenant': tenant, 'form': form, 'grades': grades, 'logo_url': tenant.school_logo.url if tenant.school_logo else None}
...
def edit_student(request, schema_name, student_id):
    ...
    with schema_context(schema_name):
        ...
        grades = FeeStructure.objects.values_list('grade', flat=True).distinct()
        context = {'tenant': tenant, 'form': form, 'student': student, 'grades': grades, 'logo_url': tenant.school_logo.url if tenant.school_logo else None}
    return render(request, 'tenant/student_form.html', context)
