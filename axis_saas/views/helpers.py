...
def get_dashboard_context(tenant, schema_name):
    ...
    return {'tenant': tenant, 'today_collection': today_collection, 'month_collection': month_collection, 'total_revenue': total_revenue, 'total_pending': total_pending, 'defaulters_count': defaulters_count, 'total_students': total_students, 'low_stock_count': low_stock_count, 'collection_rate': round(collection_rate, 1), 'recent_payments': recent_payments, 'top_defaulters': top_defaulters, 'months_labels': months_labels, 'months_amounts': months_amounts, 'logo_url': tenant.school_logo.url if tenant.school_logo else None, 'today': today, 'start_date': first_day_month}
...
def get_student_list_context(request, schema_name):
    ...
    with schema_context(schema_name):
        ...
        if query:
            students = students.filter(Q(name__icontains=query) | Q(roll_number__icontains=query) | Q(father_name__icontains=query) | Q(father_cnic__icontains=query) | Q(parent_mobile__icontains=query) | Q(grade__icontains=query))
        if grade:
    ...
    return {'tenant': tenant, 'students': page_obj, 'grades': grades, 'sections': sections, 'status_choices': status_choices, 'search_query': query, 'total_pending_all': total_pending_all, 'total_active': total_active, 'logo_url': tenant.school_logo.url if tenant.school_logo else None}
...
def get_student_profile_context(request, schema_name, student_id):
    ...
    with schema_context(schema_name):
        ...
        for p in payments_qs_all:
            ...
            payment_list.append({'payment': p, 'fee_paid': fee_paid, 'total_due_before': total_due_before, 'remaining_balance': remaining_balance})
        ...
        return {'tenant': tenant, 'student': student, 'fee_records': fee_records, 'payments': page_obj, 'total_fee': total_fee, 'total_paid': total_paid, 'pending_total': pending_total, 'item_purchase_total': item_purchase_total, 'current_month': current_month, 'current_year': current_year, 'logo_url': tenant.school_logo.url if tenant.school_logo else None, 'search_date': search_date}
...
def extract_item_sales_from_remarks(remarks):
    ...
    from decimal import Decimal
    text = remarks or ''
    marker_match = re.search('items sold\s*:\s*(.*)', text, flags=re.IGNORECASE)
    if not marker_match:
        marker_match = re.search('items sold\s+(.*)', text, flags=re.IGNORECASE)
    candidate_text = marker_match.group(1) if marker_match else text
    pattern = re.compile('(?P<name>.+?)\s*x\s*(?P<qty>\d+)\s*@\s*₹\s*(?P<price>\d+(?:\.\d+)?)\s*=\s*₹\s*(?P<total>\d+(?:\.\d+)?)', flags=re.IGNORECASE)
    items = []
    for chunk in re.split(';\s*', candidate_text):
        ...
        if not match:
            continue
        items.append({'name': match.group('name').strip(), 'quantity': int(match.group('qty')), 'unit_price': Decimal(match.group('price')), 'line_total': Decimal(match.group('total')), 'raw': chunk})
    return items
...
extract_item_sales_from_remarks = extract_item_sales_from_remarks
